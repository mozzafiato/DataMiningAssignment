    ...
    def training(self):
        ...
        for epoch in range(self.args.nb_epochs):
            ...
            if loss < best:
                best = loss
                cnt_wait = 0
                torch.save(model.state_dict(), '../Task2/embeddings/best_{}_{}.pkl'.format(self.args.dataset, self.args.embedder))
            else:
                cnt_wait += 1
        ...
        # Evaluation
        model.eval()
        torch.save(model.H.data.detach(), "embeddings/embeds_{}_{}.pkl".format(self.args.dataset, self.args.embedder))
